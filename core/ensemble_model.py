...
class EnsembleModel:
    ...
    def _init_models(self):
        m = {}
        if XGB_AVAILABLE:
            m['xgb'] = xgb.XGBClassifier(n_estimators=200, max_depth=4, learning_rate=0.05,
                subsample=0.8, colsample_bytree=0.8, use_label_encoder=False,
                eval_metric='mlogloss', random_state=42, n_jobs=-1, verbosity=0)
        if LGB_AVAILABLE:
            m['lgb'] = lgb.LGBMClassifier(n_estimators=200, max_depth=4, learning_rate=0.05,
                subsample=0.8, colsample_bytree=0.8, random_state=42, n_jobs=-1, verbose=-1)
        if not m:
            from sklearn.ensemble import RandomForestClassifier
            m['rf'] = RandomForestClassifier(n_estimators=200, max_depth=6, random_state=42, n_jobs=-1)
        return m
    ...
    def fit(self, df: pd.DataFrame):
        ...
        mask = feat.notna().all(axis=1) & ~np.isnan(labels)
        X = feat[mask].values; y = labels[mask]
        ...
        for name, model in self.models.items():
            ...
            if hasattr(model, 'feature_importances_'):
                self.feature_importance_[name] = dict(zip(self.FEATURE_COLS, model.feature_importances_))
    ...
    def predict_proba_all(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        result = pd.DataFrame(0.0, index=df.index, columns=['p_short','p_hold','p_long'])
